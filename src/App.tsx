    ...
    () =>
      createTheme({
        palette: {
          ...
          // 可以在这里统一调整主色调等，以匹配第一个文件的效果
          // 例如：primary: { main: '#00ff9d' }
        },
      }),
    [darkMode]
  );
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Box
        sx={{
          minHeight: '100vh',
          // **修改 1：移除固定背景色，使用背景图片和蒙版逻辑**
          // bgcolor: 'background.default', // 保持默认主题色，但会被背景图片逻辑覆盖
          color: 'text.primary',
            ...
            <Box
              sx={{
                ...
                '&::before': {
                  ...
                  right: 0,
                  bottom: 0,
                  // **修改 2：蒙版颜色逻辑已存在，但调整透明度计算，使其更接近第一个文件（第一个文件是 1-opacity，这个文件也是，保持不变）**
        ...
        >
          ...
          >
            <Typography
              ...
              component='h1'
              fontWeight='bold'
              // **修改 3：移除颜色覆盖，使用主题默认颜色（已是 text.primary）**
              color='text.primary'
              sx={{
                  ...
                  {viewMode === 'readonly' ? (
                    ...
                    <Button
                      ...
                      sx={{
                        minWidth: 'auto',
                        fontSize: { xs: '0.75rem', sm: '0.875rem' },
                        // **修改 4：增强访客登录按钮的视觉效果 (可选)**
                        // 如果要完全复制第一个文件的浮动按钮样式，需要在这里添加：
